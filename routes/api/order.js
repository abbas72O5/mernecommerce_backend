...
const getMerchantOrderQuery = async merchantId => {
  const brandIds = await Brand.find({ merchant: merchantId }, '_id');
  const carts = await Cart.aggregate([
    ...
    {
      $match: {
        'productDoc.brand': { $in: brandIds.map(b => b._id) }
      }
    },
    {
      $group: {
  ...
  return { cart: { $in: carts.map(c => c._id) } };
};
...
router.get('/search', auth, async (req, res) => {
  try {
    ...
    } else if (req.user.role === ROLES.Merchant && req.user.merchant) {
      const merchantOrderQuery = await getMerchantOrderQuery(req.user.merchant);
      ordersDoc = await Order.find({
        _id: Mongoose.Types.ObjectId(search),
        ...merchantOrderQuery
      }).populate({
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === ROLES.Merchant && req.user.merchant) {
      query = await getMerchantOrderQuery(req.user.merchant);
    }
