...
router.post("/add", auth, role.check(ROLES.Admin), async (req, res) => {
    try {
        ...
        // Keep auth consistent: create a merchant user record at creation time.
...
router.put("/:id/active", auth, role.check(ROLES.Admin, ROLES.SuperAdmin), async (req, res) => {
    try {
        const merchantId = req.params.id;
        const update = req.body.merchant;
        console.log("[Merchant API] Toggle active request", {
            merchantId,
            requestedBy: req.user?._id,
            requestedByRole: req.user?.role,
            nextIsActive: update?.isActive,
        });
        ...
        await User.updateMany(
            { email: merchantDoc.email, role: ROLES.Merchant },
            { isActive: !!update.isActive },
        );
        console.log("[Merchant API] Toggle active success", {
            merchantId,
            isActive: merchantDoc.isActive,
        });

        res.status(200).json({
            success: true,
            message: `Merchant account has been ${
                update.isActive ? "enabled" : "disabled"
            } successfully.`,
            merchant: merchantDoc,
        });
    } catch (error) {
        console.error("[Merchant API] Toggle active failed", error);
        res.status(400).json({
            error: "Your request could not be processed. Please try again.",
        });
    }
